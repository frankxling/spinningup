...
"/home/siwflhc/anaconda3/envs/arm_gpu/bin/python /home/siwflhc/spinningup/spinup/plotSQL.py"
class ArmState(Enum):
    Reached = auto()
    InProgress = auto()
...
table_name = 'rosarmtesting' #modify place
""" create a database connection to a SQLite database """
conn = None
try:
    conn = sqlite3.connect('env_log.db', detect_types=sqlite3.PARSE_DECLTYPES)  #modify place
...
# data[3] is cum_normalised_reward
# data[4] is cum_reward
# data[5] is normalised_reward
# data[6] is arm_state
# data[7] is target_coords


dist_to_goal = np.array(data2[0])
reward = np.array(data2[1])
episode_num = np.array(data2[2])
cum_normalised_reward = np.array(data2[3])
cum_reward = np.array(data2[4])
normalised_reward = np.array(data2[5])
target_coords = np.array(data2[7])
...
sql_statement = \
    ''' 
    select min(dist_to_goal),reward,episode_num,cum_normalised_reward,cum_reward,normalised_reward,arm_state,target_coords,max(step_num)
    from rosarmtesting
    	GROUP BY episode_num 
    '''  #condition is put at where not in "inprogree" or stpe is more or what...   from the saved table  
    #WHERE arm_state == 'Collision' or arm_state == 'ApproachJointLimits'
